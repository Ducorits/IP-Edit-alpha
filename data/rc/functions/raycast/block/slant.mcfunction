...
# some other stuff
scoreboard players operation x calc = @s rayX
scoreboard players operation dy calc = @s rayY
#scoreboard players operation @s calc -= @s Zpos

# y = ax + b
scoreboard players operation temp calc = a calc
scoreboard players operation temp calc *= x calc
scoreboard players operation temp calc /= 100 calc
scoreboard players operation temp calc += b calc

scoreboard players operation y calc = temp calc

# see if pos of ray entity is within corner scores
execute if score @s rayX > @s cor0X if score @s rayY > @s cor0Y if score @s rayZ > @s cor0Z if score @s rayX < @s cor1X if score @s rayY < @s cor1Y if score @s rayZ < @s cor1Z if score @s rayY < y calc run tag @s add stop